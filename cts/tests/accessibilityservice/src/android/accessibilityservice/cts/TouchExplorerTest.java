 ...
 *
 * *用于测试触摸探索的一组测试。每个测试分派一个手势并检查
 *
 * *适当的悬停和/或触摸事件，然后是适当的可访问性事件。一些
 *
 * *测试将检查视图中的事件。
 */
...
public class TouchExplorerTest {
    ...
    public static void oneTimeSetup() {
        sInstrumentation = InstrumentationRegistry.getInstrumentation();//获取Instrumentation的对象
        // Save enabled accessibility services before disabling them so they can be re-enabled after
        // the test.//在禁用前保存已启用的可访问性服务，以便在禁用后重新启用测试。
        //ENABLED_ACCESSIBILITY_SERVICES：启用的辅助功能提供程序列表。
        ...
        // that depends on which services are enabled. //开启无障碍服务前，请先关闭所有服务，防止易剥落
        //取决于启用了哪些服务。
        InstrumentedAccessibilityService.disableAllServices();
        //UiAutomation还可借助Instrumentation框架运行
        // 通过调用Instrumentation.getUiAutomation()即可获得UiAutomation的一个实例
        //获取FLAG_DONT_SUPPRESS_ACCESSIBILITY_SERVICES 标志 UiAutomation的一个实例
    ...
    public void setUp() throws Exception {
        ...
        mActivityRule.launchActivity(null);//启动被测activity  黑格妹妹学画画3w 日常学费1w
    ...
    /** Test a slow swipe which should initiate touch exploration.测试一个缓慢的滑动，这将启动触摸探索。 */
    @Test
    @AppModeFull
